 ...
 */
function uploadPicture(requestUrl, callback, scope, allowMultiUpload, isOnlyPic){
	url = requestUrl;
	// 创建相册窗口
		...
		listeners : {
			afterrender : function(){
				addBtn.el.createChild({
		        	...
		        	add(this.dom.files[0], isOnlyPic);
		        });
			}
		}
	});
...
function add(file, isOnlyPic) {
	console.log(isOnlyPic);
	if(isOnlyPic != undefined && isOnlyPic != null){
		if(isOnlyPic && file.type.indexOf("image") < 0){
			Ext.Msg.alert("提示", "对不起，只能上传图片！");
			return;
		}
	}else{
