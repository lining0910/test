...
public class RestClientUtil {
	...
	//微信小程序appid
	private static String wxMiniProgramAppid;
	...
	public void init() {
		...
		portalHost = memberConfig.getPortalHost();
		wxMiniProgramAppid = memberConfig.getWxMiniProgramAppid();
	}
	...
	public static JSONObject notice(String categoryId) throws Exception{
		
		String url = String.format(cmsUrl, categoryId);
	...
	/**
	 * 根据用户手机号和用户会员卡号判断是否有过停卡申请记录
	 * @param mobileNum
	 * @param memberCardNo
	 * @return
	 */
	public static boolean isApply(String mobileNum, String memberCardNo){
		
		String url = portalHost + "service/rest/us.AppFeedback/collection/query?userMobile="+mobileNum+"&content="+memberCardNo+"&type=1";
		
		logger.info(ProjectConfig.PREFIX + "isApply:{}", url);
		
		String result = HttpClientUtils.getInstance().httpGet(url);
		logger.info(ProjectConfig.PREFIX + "isApply:{}", result);
		
		JSONObject resultObj = JSON.parseObject(result);
		if(!resultObj.containsKey("items"))
			throw new RuntimeException("判断是否有过停卡申请记录出错");
	 ...
	 * @throws Exception
	 */
	public static String createApply(String mobileNum, String content, String type) throws Exception{
		
		String url = portalHost + "service/rest/us.AppFeedback/collection/createNoImage";
		
		logger.info(ProjectConfig.PREFIX + "createApply:{}", url);
		
		org.json.JSONObject param = new org.json.JSONObject();
		param.put("type", type);
		param.put("content", content);
		param.put("userMobile", mobileNum);
		param.put("appId", wxMiniProgramAppid);
		param.put("isBase64Encode", "0");
		
		String result = HttpClientUtils.getInstance().httpPost(url, param);
		logger.info(ProjectConfig.PREFIX + "createApply:{}", result);
		
		JSONObject resultObj = JSON.parseObject(result);
		if("100".equals(resultObj.getString("result_code")))
			return "success";
		
		return resultObj.getString("result_desc");
	}
}	
