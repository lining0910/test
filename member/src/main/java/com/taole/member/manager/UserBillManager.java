...
public class UserBillManager {
	...
	@Transactional
	  public String consumeForMember(UserBill userbill, GoodsBill goodsBill, List<GoodsBillDetail> goodsList, MemberCard memberCard)
	  {
	    boolean isConsumeGoods = false;
	    String shopBillId = "";
	    if ((goodsBill != null) && (goodsList != null) && (goodsList.size() != 0))
	    {
	      String goodsBillNo = userbill.getUserBillNo();
	      goodsBill.setShopBillNo(goodsBillNo);
	      shopBillId = this.goodsBillManager.createGoodsBill(goodsBill);
	      for (GoodsBillDetail goodsBillDetail : goodsList)
	      {
	        Double balance = this.goodsBillDetailManager.getGoodsBalanceNum(goodsBillDetail.getShopId(), goodsBillDetail
	          .getGoodsId());
	        goodsBillDetail.setBalance(Double.valueOf(balance.doubleValue() - goodsBillDetail.getAmount().doubleValue()));
	        goodsBillDetail.setShopBillId(shopBillId);
	        this.goodsBillDetailManager.createGoodsBillDetail(goodsBillDetail);
	      }
	      isConsumeGoods = true;
	    }
	    Integer swipeAmount = userbill.getSwipeAmount();
	    if ((swipeAmount != null) && (swipeAmount.intValue() > 0))
	    {
	      if (this.memberCardManager.isFirstSwipe(memberCard))
	      {
	        memberCard.setStatus("1");
	        memberCard.setApproverTime(new Date());
	        memberCard.setDeadline(this.memberCardManager.getDeadDate(memberCard, null));
	        this.memberCardManager.updateMemberCard(memberCard);
	      }
	      userbill.setBalanceNum(Integer.valueOf(getBalanceNum(userbill.getUserCardId()) - userbill.getSwipeAmount().intValue()));
	    }
	    else
	    {
	      userbill.setSwipeAmount(Integer.valueOf(0));
	      userbill.setBalanceNum(Integer.valueOf(getBalanceNum(userbill.getUserCardId())));
	    }
	    userbill.setShopBillId(shopBillId);
