...
public class ShopCardManager {
	...
	public JSONArray allShopCard(String shopId){
		...
		for(Map.Entry<String, DictNode> entry : nodeMap.entrySet()){
			...
			JSONArray resultAry = getAllShopCard(entry.getKey(), shopId);
			cardType.put("cards", resultAry);
	...
	public JSONArray getAllShopCard(String cardType, String shopId){
		...
		for(int i=0; i<cardArray.size(); i++){
			...
			if(StringUtils.isNotBlank(beginTime) && DateUtil.compare(new Date(), DateUtil.StringToDate(beginTime, DateStyle.YYYY_MM_DD_HH_MM_SS)) >= 0){
				if(StringUtils.isBlank(endTime) || (DateUtil.compare(new Date(), DateUtil.StringToDate(endTime, DateStyle.YYYY_MM_DD_HH_MM_SS)) <= 0)){
					resultAry.add(obj);
				}
			}
		}
	...
	public JSONArray shopCardList(final ShopCardCondition condition){
		...
		for(int i=0; i<shopGoodsList.size(); i++){
			...
			Date beginTime = (Date) objs[7];
			object.put("beginTime", "");
			if(beginTime != null)
				object.put("beginTime", DateUtil.DateToString(beginTime, DateStyle.YYYY_MM_DD_HH_MM_SS));
			
			Date endTime = (Date) objs[8];
			object.put("endTime", "");
			if(endTime != null)
				object.put("endTime", DateUtil.DateToString(endTime, DateStyle.YYYY_MM_DD_HH_MM_SS));
	...
	public JSONArray shopCardListForRecharge(final ShopCardCondition condition){
		...
		for(int i=0; i<shopGoodsList.size(); i++){
			...
			object.put("money", objs[3]);
			object.put("cardImage", StringUtils.isNotBlank((String)objs[4]) ? objs[4] : memberConfig.getDefaultImage());
			object.put("imageBack", StringUtils.isNotBlank((String)objs[5]) ? objs[5] : "");
			resultAry.add(object);
		}
