...
public class HousekeeperApi {
	...
	public @ResponseBody Object checkTicket(HttpServletRequest request, HttpServletResponse response,
			...
			@ApiParam(value = "当前消费店铺ID", required = true) @RequestParam String shopId){
		...
		try {
			...
			if(StringUtils.isBlank(shopId))
				return new ServiceResult(ReturnResult.FAILURE, "所在店铺信息获取失败请重新获取");
			
			ShopInfo shopInfo = shopInfoManager.getShopInfo(shopId);
			if(shopInfo == null)
				return new ServiceResult(ReturnResult.FAILURE, "所在店铺信息获取失败请重新获取");
			
			if(!shopId.equals(userGoodsOrder.getShopId()))
				return new ServiceResult(ReturnResult.FAILURE, "该门票领取店铺与购买店铺不一致");
	...
	public @ResponseBody Object pickUpGoods(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if(!shopId.equals(userGoodsOrder.getShopId()))
				return new ServiceResult(ReturnResult.FAILURE, "该订单商品领取店铺与购买店铺不一致");
	...
	public @ResponseBody Object payByCard(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if(MemberCardStatus.OVERDUE.equals(memberCard.getStatus()))
				return new ServiceResult(ReturnResult.NOT_FOUND_OBJECT, "该会员卡已过期");
			...
			userBill.setOperatorShopId(shopId);
	...
	private int todayCardPayNum(String userCardId){
		...
		userBillCondition.setActionTypeId(TransactionType.HK);
