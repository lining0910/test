...
public class MemberCardManager {
	...
	public Date getDeadDate(MemberCard memberCard, Date deadline) {
		...
		if(deadline != null && DateUtil.compare(new Date(), deadline) <= 0){
			nowTime = deadline;
		}
	...
	//办卡
	@Transactional
	public String applyCard(User user,CardInfo cardInfo,UserBillApply userBillApply ,MemberCard memberCardReq, TradeLock tradeLock2){
	...
	public JSONArray userMemberCard(final String userId){
		...
		for(int i=0; i<list.size(); i++){
			...
			if(MemberCardStatus.T0_BE_OPEN.equals(status)){
				String totalNum = String.valueOf((Integer) objs[8]);
			...
			object.put("userAvatar", userManager.avatarFactory((String) objs[10]));
			object.put("userBirthDay", objs[11]);
			object.put("userGender", ((Integer) objs[12]) == null ? "" : String.valueOf((Integer) objs[12]));
			object.put("shopName", objs[14]);
	...
	public void rechargeSuccess(String orderId){
		...
		for(OrderService orderService : orderServices){
			...
			userBill.setConsumeMoney(orderService.getMoney());
