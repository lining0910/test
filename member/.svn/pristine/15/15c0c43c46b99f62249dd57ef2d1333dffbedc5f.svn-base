...
import com.taole.member.sql.CountUserBillSql;
...
public class ShopGoodsManager {
	...
	public JSONArray shopGoodsForHome(String shopId){
		...
		for(Map.Entry<String, DictNode> entry : dictNodeMap.entrySet()){
			...
			condition.setHomeCatalogId(entry.getKey());
...
//		dictNodeMap.remove("SPL");
	...
	public Integer shopGoodsTimeCount(final UserBillCondition condition){
		List<?> list = ((HibernateDaoSupport) userBillDao).getHibernateTemplate().executeWithNativeSession(new HibernateCallback<List<?>>() {
			@Override
			public List<?> doInHibernate(Session session) throws HibernateException, SQLException {
				String sql = CountUserBillSql.countUserBillSql(condition);
				SQLQuery sqlQuery = session.createSQLQuery(sql);
				List<?> list = sqlQuery.list();
				return list;
			}
	...
	public JSONArray shopGoodsList(final ShopGoodsCondition condition, final int start, final int limit){
		...
		for(int i=0; i<shopGoodsList.size(); i++){
			...
			object.put("image", StringUtils.isNotBlank(image) ? image : memberConfig.getDefaultImage());
			object.put("description", objs[4]);
			object.put("originalCost", objs[5]);
			//object.put("balance", objs[6]);
			Double balance = goodsBillDetailManager.getGoodsBalanceNum(condition.getShopId(), goodsId);
			object.put("balance", balance);
			object.put("maxnumber", objs[6]);
	...
	public JSONArray shopTickeyList(final ShopGoodsCondition condition, final int start, final int limit){
		...
		for(int i=0; i<shopGoodsList.size(); i++){
			...
			object.put("image", StringUtils.isNotBlank(image) ? image : memberConfig.getDefaultImage());
			object.put("description", objs[4]);
			String imageBack = (String) objs[5];
			object.put("imageBack", StringUtils.isNotBlank(imageBack) ? imageBack : memberConfig.getDefaultImage());
			object.put("originalCost", objs[6]);
			//object.put("balance", objs[6]);
			Double balance = goodsBillDetailManager.getGoodsBalanceNum(condition.getShopId(), goodsId);
			object.put("balance", balance);
			object.put("maxnumber", objs[7]);
	...
	public void buySuccess(String orderId){
		...
		if(OrderType.GOODS.equals(order.getType())){
			userBill.setActionTypeId(TransactionType.XF);
		}else {
			userBill.setActionTypeId(TransactionType.MP);
		}
